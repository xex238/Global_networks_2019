import recipient_functions as functions
import socket
import time
...
if(print_information):
    print("Часть 2. Выводим полученные данные на экран")
    print()
...
bit_mas_new, count_of_zero_errors, count_of_one_errors, count_of_two_errors = functions.Repeat_Hamming_code(Hamming_mas, length_hamming_word_input)
...
print("Время проверки и расщифровки составило", end_time - start_time, "секунд")
print()
...
sock_1.send(bytes(functions.dec_bin(count_of_zero_errors)))
sock_1.close()
...
sock_3.send(bytes(functions.dec_bin(count_of_one_errors)))
sock_3.close()
...
sock_4.send(bytes(functions.dec_bin(count_of_two_errors)))
sock_4.close()
